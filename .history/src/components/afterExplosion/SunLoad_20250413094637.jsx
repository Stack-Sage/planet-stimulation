...
const SunLoad = () => {
  ...
  const audio = new Audio(ig.bonus); // Preload audio globally
  audio.oncanplaythrough = () => setAudioLoaded(true); // Track when the audio is ready

  useEffect(() => {
    audio.load(); // Ensure audio is preloaded when the component mounts
  }, []);
  ...
  useEffect(() => {
    const timeout1 = setTimeout(() => {
      playSound(); // Play sound when the toast shows
      toast.success(
    ...
    }, 3000); // Toast will show 3 seconds after mounting
